...
extern void UnloadFontDefault(void)
{
    ...
    UnloadImage(defaultFont.texture);
...
Font LoadFontEx(const char *fileName, int fontSize, int *fontChars, int glyphCount)
{
    ...
    if (fileData != NULL)
    {
        ...
        font = LoadFontFromMemory(RIMAGE_GETFILEEXTENSION(fileName), fileData, fileSize, fontSize, fontChars, glyphCount);

        RIMAGE_FREE(fileData);
    }
...
Font LoadFontFromImage(Image image, Color key, int firstChar)
{
    ...
    font.texture = fontClear; // LoadTextureFromImage(fontClear); // Convert processed image to OpenGL texture
...
Font LoadFontFromMemory(const char *fileType, const unsigned char *fileData, int dataSize, int fontSize, int *fontChars, int glyphCount)
{
        ...
        TextIsEqual(fileExtLower, ".otf"))
    {
        ...
        if (font.glyphs != NULL)
        {
            ...
            font.texture = atlas; //LoadTextureFromImage(atlas);
...
void UnloadFont(Font font)
{
    ...
    if (font.texture.data != GetFontDefault().texture.data)
    {
        UnloadFontData(font.glyphs, font.glyphCount);
...
int MeasureText(const char *text, int fontSize)
{
    ...
    // Check if default font has been loaded
    if (GetFontDefault().texture.data != NULL)
    {
        int defaultFontSize = 10;   // Default Font chars height in pixel
        if (fontSize < defaultFontSize) fontSize = defaultFontSize;
        int spacing = fontSize/defaultFontSize;
...
Vector2 MeasureTextEx(Font font, const char *text, float fontSize, float spacing)
{
    ...
    if ((font.texture.data == NULL) || (text == NULL)) return textSize;
