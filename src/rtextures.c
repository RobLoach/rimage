/**********************************************************************************************
*
*   rtextures (modified) - Basic functions to load and draw textures
*
*   CONFIGURATION:
*
...
Image LoadImage(const char *fileName)
{
    ...
    unsigned char *fileData = RIMAGE_LOADFILEDATA(fileName, &fileSize);

    // Loading image from memory data
    if (fileData != NULL) image = LoadImageFromMemory(RIMAGE_GETFILEEXTENSION(fileName), fileData, fileSize);
...
Image LoadImageRaw(const char *fileName, int width, int height, int format, int headerSize)
{
    ...
    unsigned char *fileData = RIMAGE_LOADFILEDATA(fileName, &dataSize);
    ...
    if (fileData != NULL)
    {
        ...
        RIMAGE_MEMCPY(image.data, dataPtr, size); // Copy required data to image
        image.width = width;
        image.height = height;
        image.mipmaps = 1;
        image.format = format;
...
Image LoadImageFromMemory(const char *fileType, const unsigned char *fileData, int dataSize)
{
    ...
    if ((false)
#if defined(SUPPORT_FILEFORMAT_PNG)
        || (RIMAGE_STRCMP(fileType, ".png") == 0)
#endif
#if defined(SUPPORT_FILEFORMAT_BMP)
        || (RIMAGE_STRCMP(fileType, ".bmp") == 0)
#endif
#if defined(SUPPORT_FILEFORMAT_TGA)
        || (RIMAGE_STRCMP(fileType, ".tga") == 0)
#endif
#if defined(SUPPORT_FILEFORMAT_JPG)
        || ((RIMAGE_STRCMP(fileType, ".jpg") == 0) || (RIMAGE_STRCMP(fileType, ".jpeg") == 0))
#endif
#if defined(SUPPORT_FILEFORMAT_GIF)
        || (RIMAGE_STRCMP(fileType, ".gif") == 0)
#endif
#if defined(SUPPORT_FILEFORMAT_PIC)
...
#if defined(SUPPORT_FILEFORMAT_HDR)
    else if (RIMAGE_STRCMP(fileType, ".hdr") == 0)
    {
#if defined(STBI_REQUIRED)
        if (fileData != NULL)
        {
...
#endif
#if defined(SUPPORT_FILEFORMAT_QOI)
    else if (RIMAGE_STRCMP(fileType, ".qoi") == 0)
    {
        qoi_desc desc = { 0 };
...
bool ExportImage(Image image, const char *fileName)
{
    ...
    if (IsFileExtension(fileName, ".png"))
    {
        ...
        success = RIMAGE_SAVEFILEDATA(fileName, fileData, dataSize);
        RL_FREE(fileData);
    }
#else
    if (false) { }
    ...
    else if (IsFileExtension(fileName, ".raw"))
    {
        ...
        success = RIMAGE_SAVEFILEDATA(fileName, image.data, GetPixelDataSize(image.width, image.height, image.format));
    }
...
Image ImageFromImage(Image image, Rectangle rec)
{
    ...
    {
        RIMAGE_MEMCPY(((unsigned char *)result.data) + y*(int)rec.width*bytesPerPixel, ((unsigned char *)image.data) + ((y + (int)rec.y)*image.width + (int)rec.x)*bytesPerPixel, (int)rec.width*bytesPerPixel);
    }
...
Image ImageTextEx(Font font, const char *text, float fontSize, float spacing, Color tint)
{
    Image imText = { 0 };
#if defined(SUPPORT_MODULE_RTEXT)
    int size = (int)RIMAGE_STRLEN(text);   // Get size in bytes of text
...
/*
================================================
Texture Drawing functions: Not needed for rimage
================================================
